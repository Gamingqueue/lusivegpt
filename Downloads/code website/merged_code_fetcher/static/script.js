...
let SERVICE_TYPE = window.SERVICE_TYPE || 'netflix'; // Default to netflix if not set
...
function handleKeyPress(event) {
    if (event.key === 'Enter') {
        ...
        if (!isLoading && keyInput && keyInput.value.trim()) {
            fetchCode();
        }
    }
}
...
async function validateKey(key) {
    try {
        const response = await fetch('/validate-key', {
            ...
            body: JSON.stringify({ key: key, service: SERVICE_TYPE })
        });
...
async function fetchCode() {
    ...
    try {
        ...
        if (response.ok && data.success) {
            displayCode(data.code, data.remaining_uses);
            showToast(`${SERVICE_TYPE.charAt(0).toUpperCase() + SERVICE_TYPE.slice(1)} code retrieved successfully!`, 'success');
        } else {
            displayError(data.error || `Failed to fetch ${SERVICE_TYPE} code`);
            showToast(data.error || `Failed to fetch ${SERVICE_TYPE} code`, 'error');
        }
 ...
 */
function displayCode(code, remainingUses) {
    ...
    codeContent.innerHTML = `
        <div class="code-label">
            ...
            Your ${SERVICE_TYPE.charAt(0).toUpperCase() + SERVICE_TYPE.slice(1)} Code
        </div>
...
function resetResultDisplay() {
    ...
    resultDisplay.innerHTML = `
        <div class="result-placeholder">
            <i class="fas fa-ticket-alt"></i>
            <span>Your ${SERVICE_TYPE.charAt(0).toUpperCase() + SERVICE_TYPE.slice(1)} code will appear here</span>
        </div>
    `;
}
...
function resetInputValidation() {
    if (!keyInput || !inputValidation) return;
