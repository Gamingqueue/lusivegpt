...
async function fetchCode() {
    ...
    try {
        ...
        if (response.ok && data.success) {
            displayCode(data.code, data.usage_info);
            showToast('2FA code generated successfully!', 'success');
        } else {
            displayError(data.error || 'Failed to generate code');
...
/**
 * Display 2FA code with animation and usage info
 */
function displayCode(code, usageInfo = null) {
    ...
    let usageInfoHtml = '';
    if (usageInfo) {
        const { max_uses, usage_count, remaining_uses } = usageInfo;
        if (max_uses === -1) {
            usageInfoHtml = `
                <div class="usage-info">
                    <i class="fas fa-infinity"></i>
                    <span>Unlimited uses (Used ${usage_count} times)</span>
                </div>
            `;
        } else {
            usageInfoHtml = `
                <div class="usage-info">
                    ...
                    <span>${remaining_uses} uses remaining (${usage_count}/${max_uses} used)</span>
                </div>
            `;
        }
    }
...
function resetResultDisplay() {
    ...
    resultDisplay.innerHTML = `
        <div class="result-placeholder">
            <i class="fas fa-ticket-alt"></i>
            <span>Your 2FA code will appear here</span>
        </div>
    `;
}
